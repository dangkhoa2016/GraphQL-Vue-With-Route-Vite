		...
		<div class="row">
			<div class="col-lg-4">
				<div class="card mb-3">
					<div class="row g-0">
						<div
							class="col-md-4 col-lg-12 text-lg-center text-center text-md-start"
						>
							<img
								:src="avatarUrl"
						...
						<div class="col-md-8 col-lg-12">
							<div class="p-3 border-bottom">
								<h5
									class="mb-0"
									:class="{ 'skeleton profile-name m-0': loadingCat }"
								>
									{{ loadingCat ? '' : cat?.name }}
								</h5>
							</div>
							...
							<ul class="list-group list-group-flush">
								<li class="list-group-item d-flex align-items-center">
									...
									<span
										...
										:class="{ 'profile-short skeleton': loadingCat }"
									>
							...
							>
								...
								<span class="d-flex align-items-center">
									<svg class="d-inline-block cat-stats me-1">
										<use xlink:href="#likes"></use>
									</svg>
									<span
										:class="
											loadingCat
												? 'd-inline-block skeleton stats-count'
							...
							<div class="card-footer py-3 d-flex align-items-center">
								...
								<small
									class="ms-2"
									:class="{
										'd-inline-block skeleton profile-name': loadingCat,
									}"
								>
									<a
										href="#"
										@click.prevent="openOwnerDetail"
			...
			<div class="col-lg-8">
				...
				<simple-accordion
					@toggle="handleTransition"
					:content="catTabs"
					:disabled="loadingCat || !!loadCatError"
				>
					<template #about>
						<p
							class="p-3"
							:class="loadingCat ? 'skeleton profile-bio' : 'bg-body-secondary'"
						>
							{{ loadingCat ? '' : cat?.description || 'No description' }}
						</p>
						<transition name="fade-slide-up" appear :duration="600">
							<CatInfoLong
								v-if="showCatInfoLong"
								:cat="cat"
								:loading="loadingCat"
							/>
						</transition>
					...
					<template #photos>
						<PhotosByCat :cat-id="cat?.id" v-model="photosPageIndex" />
					</template>
				</simple-accordion>
			</div>
		</div>
...
<script setup>
	...
	import { calculateAge, defaultImage } from '@/libs/catHelpers.mjs';
	import {
		useCatStore,
		fetchCat,
		setLoadCatError,
	} from '@/stores/catsStore.mjs';

	import PhotosByCat from '@/components/PhotosByCat.vue';
	import SimpleAccordion from '@/components/SimpleAccordion.vue';
	import CatInfoLong from '@/components/CatInfoLong.vue';
	import ErrorLoadCats from '@/components/ErrorLoadCats.vue';
	import ToggleLikeCat from '@/components/ToggleLikeCat.vue';
	...
	const isOwner = computed(() => {
		return (
			cat.value?.user_id?.toString() === authInfo.value?.user?.id?.toString()
		);
	});
	...
	const loadCat = () => {
		...
		const pendingLoadCatId = router.currentRoute.value.params.id;
		if (!pendingLoadCatId) return;
	...
	const openOwnerDetail = () => {
		...
		router.push({ name: 'UserDetail', params: { id: cat.value?.user_id } });
	};
	...
	onMounted(() => {
		if (!isAuthenticated.value) {
			emitter.emit('toast', {
				title: 'Notice',
				message: 'Please login to see cat details.',
			});
			return;
		}
