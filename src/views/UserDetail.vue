<template>
  ...
  <div class="container">
    <ErrorLoadUsers v-if="loadUserError" :message="loadUserError"
      :show-reload-button="true" :load-details='true' @reload="reloadUser" />

    <UserProfile :loading="loadingUser" :user="user"
      :show-toggle-follow="showToggleFollow" :is-admin="isAdmin" />

    <SimpleTabs :disabled="loadingUser || (loadUserError && loadUserError.length > 0)"
      class="mt-4" :tabs="tabs">
      <template #about-tab>
      ...
      <template #photos-tab>
        <PhotosByUser :user-id="user?.id" v-model='photoPageIndex' :is-admin="isAdmin" />
      </template>
    </SimpleTabs>
  </div>
...
<script setup>
  ...
  import { useUserStore, fetchUser, setLoadUserError } from '@/stores/usersStore.mjs';
  ...
  const reloadUser = () => {
    ...
    if (!pendingLoadUserId.value)
      return;

    fetchUser(pendingLoadUserId.value, `
  ...
  watch(() => loadUserError.value, (newVal/*, oldVal*/) => {
