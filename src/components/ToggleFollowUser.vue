...
<script setup>
	...
	import {
		...
		follow,
		unfollow,
	} from '@/stores/toggleFollowsStore.mjs';
	...
	const btnClass = (cls) =>
		computed(() => {
			...
			else return props.user?.id ? cls : 'btn-secondary';
		});
	...
	const btnText = (isFollow) =>
		computed(() => {
			...
			if (isFollow) return isTogglingFollow.value ? 'Following...' : 'Follow';
			else return isTogglingUnfollow.value ? 'Unfollowing...' : 'Unfollow';
		});
	...
	const handleClick = async (isFollow) => {
		...
		if (!token) {
			emitter.emit('toast', {
				title: 'Notice',
				message: `Please login to ${isFollow ? 'follow' : 'unfollow'} user`,
			});
			return;
		}

		try {
			if (isFollow) await follow(props.user?.id, token);
			else await unfollow(props.user?.id, token);
		} catch (err) {
			if (
		...
		if (unfollowError.value) handleActionError(unfollowError.value);
		else if (followError.value) handleActionError(followError.value);
	};
