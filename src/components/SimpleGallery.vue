<template>
	<div :id="galleryId" class="row" :class="mainClass">
		<PhotoBlock
			...
			:itemBgClass="getBgClass(key)"
			:showStats="showStats"
			:image="image"
			@add-photo-stats="handleAddPhotoStats"
...
<script setup>
	...
	import { getStringWidth, clonePhotoStats } from '@/libs/helpers.mjs';
	...
	const getBgClass = (key) => {
		return [
			props.figureClass,
			`alert-${bootstrapColors[key % bootstrapColors.length]}`,
		].join(' ');
	};
	...
	const onInitLightbox = (el, pswp) => {
		pswp.on('change', () => {
			...
			if (currSlideElement) {
				const figcaption =
					currSlideElement.parentElement.querySelector('figcaption');
				if (figcaption) {
			...
			if (description) {
				...
				if (width > screenWidth) el.style.maxWidth = 'none';
				else el.style.removeProperty('max-width');
			}

			if (!el.classList.contains('translate-middle-x')) {
				const cssClasses =
					'text-white rounded-2 position-absolute start-50 translate-middle-x'.split(
						' ',
					);
				for (const className of cssClasses) el.classList.add(className);
			}

			if (PhotoStatsArr.value.length > pswp.currIndex)
				photoStatsClone = clonePhotoStats(
					el,
					PhotoStatsArr.value[pswp.currIndex],
				);
		});
	};

	const handleClick = (event) => {
		if (!lightbox) event.preventDefault();
	};
