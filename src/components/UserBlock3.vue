    ...
    <div class="profile-wrapper3 p-2 position-relative">
      ...
      <div class="profile text-center">
        <a href='#' class='d-inline-block position-relative'
          @click.prevent="openDetail" :title="`View ${user?.name}'s profile`">
          <div v-if="isProUser(user)" class="check position-absolute">
            <svg class=""><use xlink:href="#check2"></use></svg>
          </div>
          <img :src="avatarUrl" width="150" class="d-flex thumbnail mx-auto mb-2 rounded-circle skeleton-img"
            :alt="user?.name" @load="onImageLoaded($event.target)">
        </a>
        <h3 class="name text-center">{{ user?.name }}</h3>
        <p class="title mb-2 text-center text-uppercase">{{ user?.job_title }}</p>
        <p class="description text-center">{{ user?.country }}</p>
        <div class='gap-2 btn-actions d-flex flex-sm-column align-items-center justify-content-between'>
          <ToggleFollowUser :user="user" :loading="loading" />
          <button class="btn btn-sm" :class="{ 'skeleton' : loading }" style="background: var(--bs-secondary);"
...
<script setup>
  ...
  const { User3: disabledClass } = disabledClasses;
	...
	const showDisabled = computed(() => {
		let result = false;
		if (props.isAdmin)
		  result = props.user?.status === false;
		return result;
	});
