<template>
  ...
  <button v-if="isLiked(type, image.id)"
  ...
  <button v-else class="btn btn-sm ms-2 d-flex btn-outline-warning btn-like-photo"
...
<script setup>
  ...
  const graphqlFields = 'id, title, url, status, likes_count, cat_id, owner { id, name, likes_count }';
  ...
  const toggleLikePhoto = () => {
    toggleLike(type, props.image?.id, authInfo.value?.token, graphqlFields).catch(err => {
      /*
  ...
  const toggleUnlikePhoto = () => {
    toggleUnlike(type, props.image?.id, authInfo.value?.token, graphqlFields).catch(err => {
      /*
      if (err.name === 'MaxUnlikeRequestReached' || err.name === 'MaxLikeRequestReached') {
        emitter.emit('toast', { title: 'Notice', message: err.message });
        return;
