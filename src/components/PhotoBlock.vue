<template>
  ...
  <div :class="wrapperClass">
    <figure class="d-flex flex-column rounded alert shadow-sm h-100" :class='itemBgClass'>
      <a data-cropped='true' @click.prevent="(ev) => emits('show', ev)"
        :data-pswp-width="image.width"
        :data-pswp-height="image.height"
        :href="image.largeURL" class='d-flex'
        rel='noopener noreferrer' target='_blank'
      >
        <img :src="image.thumbnailURL" :alt="image.title" class="w-100 card-img-top">
      </a>
      <div v-if='showDisabled' :class="disabledClass">Disabled</div>

      <PhotoStats v-if="showStats" :image="image"
        :ref="(el) => emits('add-photo-stats', el)"
        :show-like="!isOwner" />

      <figcaption class="mt-3 card-img-bottom">
        <h2 class="h5 font-weight-bold mb-2 font-italic" :title="image.title">{{ image.title }}</h2>
        <p class="mb-0 text-small text-muted font-italic crop-text-2"
          :title="image.description">{{ image.description }}</p>
      </figcaption>

      <div v-if="isAdmin" class="d-flex justify-content-center mt-auto">
        <ToggleStatus :status="image.status"
          :model-id="image.id" model-type="Photo" />
      </div>
    </figure>
  </div>
...
<script setup>
  ...
  const showDisabled = computed(() => {
    ...
    if (isOwner.value || props.isAdmin)
      result = props.image?.status === false;
    return result;
  });
