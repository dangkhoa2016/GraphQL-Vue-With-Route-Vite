<template>
  ...
  <header class=''>
    <!-- Fixed navbar -->
    <nav id='header-nav' class="hide-to-top navbar navbar-expand-md navbar-light fixed-top bg-light">
      <div class="container">
        ...
        <div class="collapse navbar-collapse" id="navbarCollapse">
          <ul class="navbar-nav me-auto mb-2 mb-md-0">
            <li v-for="(link, index) in navLinks" :key="index"
              ...
              <template v-if="link.dropdown">
                ...
                :disabled="link.disabled" @click.prevent='handleLinkClick(link.event)'
                :class="{ disabled: link.disabled }" data-bs-toggle="dropdown"
                :active-class="dropdownActive(link) ? 'text-primary' : ''" exact-active-class=""
                role="button" aria-expanded="false">
                  {{ link.text }}
                </router-link>
                <ul class="dropdown-menu" aria-labelledby="navbarDropdown">
                  <li v-for="(subLink, subIndex) in link.subLinks" :key="subIndex">
                    <router-link :to="{ name: subLink.event }" class="dropdown-item"
              ...
              <router-link v-else :to="{ name: link.event }"
                custom
                v-slot="{ href, isActive, }"
              >
                <a class="nav-link"
                  :class="{ disabled: link.disabled, 'text-primary': isActive || isActiveLink(link.event) }"
          ...
          <div class="ms-auto d-flex align-items-center">
            <template v-if='isAuthenticated'>
              <a title='go to your profile' href='#' @click.prevent='showProfile'>
                <strong>{{ authInfo?.user?.name }}</strong>
              </a>
              <a class="ms-2 btn btn-outline-danger" href='#' @click.prevent='logout'>Sign out</a>
            </template>
            <router-link v-else :to="{ name: 'Login', query: returnQuery }"
              class="btn btn-outline-success">Sign in</router-link>
          </div>
        </div>
      </div>
    </nav>
...
<script setup>
  ...
  const showProfile = () => {
    setTransition();
    router.push({ name: 'UserDetail', params: { id: authInfo.value.user?.id } });
  };
  ...
  const handleSubLinkClick = (ev) => {
    if (ev.target.parentElement.parentElement.classList.contains('dropdown-menu'))
