	...
	<div class="mt-4">
		...
		>
			<a
				...
				@click.prevent="emits('open-detail', user?.id)"
			...
			>
				...
				<img
					:src="avatarUrl"
					width="150"
					class="d-flex thumbnail mx-auto mb-2 rounded-circle"
			...
			<div
				class="ms-md-3 mt-3 text-center mt-md-0 ms-0 mw-75 mw-md-50 text-md-start"
			>
				<h5
					class="mt-2 mb-0 profile-name text-info"
					:title="user?.name"
					:class="{ skeleton: loading }"
				>
					{{ loading ? '' : user?.name }}
				</h5>
				<span class="d-block mt-2 profile-job" :class="{ skeleton: loading }">
					{{ loading ? '' : user?.job_title }}
				</span>
				<div
					class="d-flex mt-4 flex-column gap-2 justify-content-between align-items-center"
				>
					<div v-if="loading" class="skeleton profile-job"></div>
					<div v-else-if="!loadUserError">
						Joined:
						<span class="rounded bg-info px-2 d-inline-block">
							{{ loading ? '' : formatDateTime(user?.created_at) }}
						</span>
					</div>
				</div>
			</div>
		...
		<div class="mt-2">
			...
			<div class="text-center">
				<ToggleFollowUser
					:user="user"
					:loading="loading || !!loadUserError"
					btn-follow-class="btn-outline-primary"
					btn-unfollow-class="btn-danger"
				/>
				<button
					class="btn btn-success px-4 ms-3 btn-sm"
					@click="emits('open-detail', user.id)"
					:title="`View ${user?.name}'s profile`"
					:class="{ skeleton: loading }"
					:disabled="loading || !!loadUserError"
				>
...
<script setup>
	...
	import { isProUser, defaultImage } from '@/libs/userHelpers.mjs';
