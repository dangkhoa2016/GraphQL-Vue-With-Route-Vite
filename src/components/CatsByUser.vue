<template>
  ...
  <div class="mt-4" :id='`cats-by-user-${userId}`'>
    ...
    <transition v-else :name="noCats ? '' : 'just-fade'" mode="out-in">
      ...
      <div v-else class=''>
        <CatsBlock :data="cats"
          :current-user-id='authInfo.user?.id' :is-admin='isAdmin' />
...
<script setup>
  ...
  import { useCatStore, fetchCatsByUser, setLoadCatsError, } from '@/stores/catsStore.mjs';
  ...
  const noCats = computed(() => (loaded.value && !(cats.value && cats.value.length > 0)));
  ...
  onMounted(() => {
    if (!isAuthenticated.value) {
      emitter.emit('toast', { title: 'Notice', message: 'Please login to see user cats.' });
