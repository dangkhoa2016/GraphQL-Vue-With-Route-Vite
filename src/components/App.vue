<template>
  ...
  <main class="d-flex flex-column h-100">
    ...
    <div class='my-auto py-4'>
      ...
      <ErrorLoadComponent v-else-if="isErrorLoadComponent" :component-path="componentName" />
      <router-view v-else v-slot="{ Component, route }">
        <transition :name="transitionName" mode="out-in"
          :data-name="route.name"
          :duration="{ enter: 500, leave: 500 }" appear>
...
<script setup>
  ...
  import { onMounted, onBeforeUnmount,
    defineAsyncComponent, ref, watch, computed, } from 'vue';
  ...
  const isLoadingComponent = computed(() => loadingComponent.value === 'loading');
  const isErrorLoadComponent = computed(() => loadingComponent.value === 'error');
  ...
  watch(() => authInfo.value, (newVal) => {
    ...
    if (router.currentRoute.value.name !== 'Login'
      ...
      toaster.value.pushToast('Notice', 'You need to login to view this page.');
